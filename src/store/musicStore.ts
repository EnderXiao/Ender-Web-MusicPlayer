...
export const MusicStore = defineStore({
        ...
        updateShuffleSongs() {
            ...
            this.songs.forEach((value, index) => {
                if (singerMap.has(value.singer)) {
                    ...
                    maxLen = Math.max(
                        maxLen,
                        singerMap.get(value.singer)?.length as number
                    );
                } else {
            ...
            for (let key of keyList) {
                ...
                // 更新为新序列
                if (newList.length !== 0) value = [...newList];
            }
            ...
            while (step < maxLen) {
                ...
                if (newSongList.length > 0) {
                    ...
                    if (
                        newSongList[newSongList.length - 1].singer ===
                        tmpList[0].singer
                    ) {
                        [tmpList[0], tmpList[tmpList.length - 1]] = [
    ...
    getters: {
        ...
        getCurrentRandomSong: (state): Music =>
            state.shuffleSongs[state.currentIndex],
    },
